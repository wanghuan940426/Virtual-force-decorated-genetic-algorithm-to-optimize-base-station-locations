...
class GeneticAlgorithm:
    ...
    def ga(self):
        ...
        y = [0] * self.max_gen
        self.record.write_cover_rate([0, round(self.g_best.y,4), round(self.g_best.y,4), round(self.g_best.y,4), round(self.g_best.y1,4), round(self.g_best.y2,4), round(self.g_best.y3,4), round(self.g_best.y,4)])
        for i in range(self.max_gen):
            ...
            if (i+1) % self.GA_step == 0:
                ...
                self.for_max_dis = []
                for _tmp in self.ori_point_index:
                    self.for_max_dis.append(self.scence.cost[_tmp]+self.scence.accept[_tmp])
    ...
    def init_pop(self):
        ...
        for i in range(self.pop_size):
            chromosome = Chromosome(M, N, self.scence, self.alpha1, self.alpha2, self.alpha3)
    ...
    def cal_max_min_mean(self, i):
        ...
        value = [i, round(np.min(all_cov),4), round(np.mean(all_cov),4), round(np.max(all_cov),4), round(self.g_best.y1,4), round(self.g_best.y2,4), round(self.g_best.y3,4), round(self.g_best.y,4)]
    ...
    def vfa(self):
        ...
        for i in range(self.N):
            ...
            newpoint = self.cal_remove_distance(i, point, force_sum)
    ...
    def cal_remove_distance(self, i, point, force_sum):
        maxdis = 0.3
        if self.for_max_dis[i] ==2:
            maxdis=0.1
        if self.for_max_dis[i] == 1.5:
            maxdis = 0.2
        if self.for_max_dis[i] == 1:
            maxdis = 0.3
        if self.for_max_dis[i] == 0.5:
            maxdis = 0.4
        if self.for_max_dis[i] == 0:
            maxdis = 0.5
        ...
        if fs != 0:
            new_point_ideal = [0, 0]
            new_point_ideal[0] = point[0] + force_sum[0] / fs * maxdis * math.exp(-1 / fs)
            new_point_ideal[1] = point[1] + force_sum[1] / fs * maxdis * math.exp(-1 / fs)
        else:
...
if __name__ == '__main__':
    ...
    alpha = [(0.92,0.04,0.04)]
    for tmp_alpha in alpha:
        alpha1, alpha2, alpha3 = tmp_alpha
        print(str(tmp_alpha))

        seed_number = 55
        random.seed(seed_number)
        file_path_name = './record1/'+str(alpha1)+'_'+str(alpha2)+'_'+str(alpha3)+'.txt'
